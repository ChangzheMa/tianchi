...
# ── 处理逻辑版本号 ──
#   每当"实际数据处理逻辑"改动、使同一份数据的最终结果发生变化时，手动改这里（如 v1→v2）。
#   仅用于 submit_<日期>_<版本>.zip 命名与结果溯源，不参与任何计算。
#   可复现性由固定 RANDOM_SEED（task1/task2 所有 KMeans random_state 一致）+ 确定性特征/校准保证，
#   与本版本号无关：版本号只是"该结果出自哪版代码"的标签，不改 seed 就算不动版本号也照样复现。
VERSION = 'v1'
